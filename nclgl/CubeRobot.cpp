...
CubeRobot::CubeRobot(Mesh* cube) {
	...
	SceneNode* body = new SceneNode(cube, Vector4(1, 0, 0, 1));		// set the colour and mesh
	body->SetModelScale(Vector3(10, 15, 5));						//sets size of individual part
	body->SetTransform(Matrix4::Translation(Vector3(0, 35, 0)));	//since this is the child of root node and nothing else
	AddChild(body);													//the transform is relative to that 

	/*SceneNode* hips = new SceneNode(cube, Vector4(1, 1, 0, 1));
	hips->SetModelScale(Vector3(10, 5, 5));
	hips->SetTransform(Matrix4::Translation(Vector3(0, 25, 0)));
	AddChild(hips);*/

	head = new SceneNode(cube, Vector4(0, 1, 0, 1));
	head->SetModelScale(Vector3(5, 5, 5));
	head->SetTransform(Matrix4::Translation(Vector3(0, 30, 0)));	//transform here is relative to the parent node
	body->AddChild(head);											//aka body
