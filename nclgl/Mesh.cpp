...
Mesh* Mesh::GenerateQuad() {
	...
	Mesh* q = new Mesh();
	q->numVertices = 4;
	q->type = GL_TRIANGLE_STRIP;

	q->vertices			= new Vector3[q->numVertices];
	q->textureCoords	= new Vector2[q->numVertices];
	...
	q->textureCoords[0] = Vector2(0.0f, 1.0f);
	q->textureCoords[1] = Vector2(0.0f, 0.0f);
	q->textureCoords[2] = Vector2(1.0f, 1.0f);
	q->textureCoords[3] = Vector2(1.0f, 0.0f);

	for (int i = 0; i < 4; i++) {
		q->colours[i] = Vector4(1.0f, 1.0f, 1.0f, 1.0f);
	}

	q->normals  = new Vector3[q->numVertices];
	q->tangents = new Vector4[q->numVertices];

	for (int i = 0; i < 4; ++i) {
		q->colours[i]  = Vector4(1.0f, 1.0f, 1.0f, 1.0f);
		q->normals[i]  = Vector3(0.0f, 0.0f, -1.0f);
		q->tangents[i] = Vector4(1.0f, 0.0f, 0.0f, 1.0f);
	}
...
Mesh* Mesh::GenerateQuad(Vector2 screenPos, float height, float width) {
	...
	q->vertices[0] = Vector3(0.5f, 0.5f, 0.0f);
	q->vertices[1] = Vector3(0.5f, -1.0f, 0.0f);
	q->vertices[2] = Vector3(1.0f, 0.5f, 0.0f);
	q->vertices[3] = Vector3(1.0f, -1.0f, 0.0f);
